...
class AnnotationCampaignContextFilter:
    ...
    @classmethod
    def get_node_or_fail(cls, context: Request, pk: int) -> AnnotationCampaign:
    ...
    @classmethod
    def get_edit_node_or_fail(cls, context: Request, pk: int) -> AnnotationCampaign:
        """Get node with edit rights or fail depending on the context"""
        campaign: AnnotationCampaign = cls.get_node_or_fail(context, pk)
        if not (
