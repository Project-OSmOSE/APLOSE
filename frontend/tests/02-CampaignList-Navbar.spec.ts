...
const STEP = {
    ...
    cannotAccessStorage: (page: Page) =>
        test.step('Cannot access storage', async () => {
            await expect(page.navbar.storageButton).not.toBeVisible()
        }),
    accessStorage: (page: Page) =>
        test.step('Access storage', async () => {
            await page.navbar.storageButton.click()
            await expect(page.storage.title).toBeVisible()
        }),
    canLogout: (page: Page) =>
