...
export const useSpectrogramTiles = ({
    ...
    useEffect(() => {
        ...
        const viewportX = canvasRef.current.parentElement!.parentElement!.scrollLeft;
        const tilesPerZoom = Math.pow(2, zoomLevel);
        const startTileIdx = Math.floor(viewportX / SpectrogramDimensions.width);
        const endTileIdx = Math.ceil((viewportX + SpectrogramDimensions.width) / SpectrogramDimensions.width);
    ...
    }, [ canvasRef.current, zoomLevel, canvasRef.current?.parentElement?.parentElement?.scrollLeft ]);
