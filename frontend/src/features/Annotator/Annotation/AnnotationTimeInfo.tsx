...
import { AltArrowRightLinearIcon, ClockCircleLinearIcon } from '@solar-icons/react';
...
export const AnnotationTimeInfo: React.FC<{ annotation: Annotation }> = ({ annotation }) => {
    ...
    return <div className={ styles.info }>
        <ClockCircleLinearIcon size={ 20 } className={ styles.mainIcon }/>
      ...
      { formatTime(annotation.startTime!, true) }
            { annotation.type === AnnotationType.Box && <Fragment>
                { NBSP }<AltArrowRightLinearIcon size={ 16 }/> { formatTime(annotation.endTime!, true) }
            </Fragment> }
    </span>
