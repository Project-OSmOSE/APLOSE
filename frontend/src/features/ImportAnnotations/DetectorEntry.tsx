...
import { CheckboxChangeEventDetail, IonCheckbox, IonIcon } from '@ionic/react';
...
export const DetectorEntry: React.FC<{
    ...
    return <div className={ [ styles.detectorEntry, isKnown ? '' : styles.unknown ].join(' ') }>
        ...
        { isKnown && <Note color="medium">Already in database</Note> }

        { !isKnown && <div className={ styles.unknown }>
            <Note color={ !isKnown && !isUpdated ? 'danger' : 'medium' }>Unknown detector</Note>
