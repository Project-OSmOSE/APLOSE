...
import { DatasetName } from "./DatasetInfo.tsx";
import { DisplayDataAPI } from "./api";

export const DatasetTable: React.FC = () => {

  const { data, error, isFetching } = DisplayDataAPI.endpoints.getDatasets.useQuery();
  const datasets = useMemo(() => {
    return data?.allDatasets?.results.filter(d => d !== null) ?? []
  }, [ data ]);
  ...
  if (!datasets) return <IonNote color='medium' style={ { textAlign: 'center' } }>
    No datasets
  </IonNote>
